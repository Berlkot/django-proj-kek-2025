  ...
  <div class="container mx-auto px-4 py-8 md:py-12">
    ...
    >
      ...
      <div>
        ...
        <p v-if="formErrors.main_image" class="text-red-500 text-xs mt-1">
          {{ formErrors.main_image.join(', ') }}
        </p>
      </div>
      ...
      <div>
        ...
        <p v-if="formErrors.categories" class="text-red-500 text-xs mt-1">
          {{ formErrors.categories.join(', ') }}
        </p>
      </div>
...
const fetchArticleData = async () => {
  ...
  try {
    ...
    initialError.value = axios.isAxiosError(err)
...
const handleSubmit = async () => {
  ...
  try {
    let response
    if (articleId.value) {
      response = await axios.patch<ArticleDetail>(
        `${API_BASE_URL}/articles/${articleId.value}/`,
        data,
        {
    ...
    if (axios.isAxiosError(err) && err.response) {
      if (err.response.status === 400 && typeof err.response.data === 'object') {
        ...
        submitError.value =
          err.response.data?.detail || `Произошла ошибка (${err.response.status}).`
      }
    } else {
...
onMounted(() => {
  ...
  if (
    !authStore.user ||
    !authStore.user.is_staff /* && !authStore.user.role?.can_manage_articles */
  ) {
  }
})
</script>
