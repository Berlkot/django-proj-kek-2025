      ...
      <div class="container mx-auto px-4 flex flex-col md:flex-row items-center relative z-10">
        <div class="md:w-1/2 text-center md:text-left mb-10 md:mb-0 md:pr-8">
          ...
          <form @submit.prevent="performSearch" class="flex max-w-md mx-auto md:mx-0">
            <input type="text" v-model="searchQueryHomepage" placeholder="Поиск животных..."
        ...
        <div class="md:w-1/2 flex justify-center md:justify-end self-end">
          <img src="/static/images/hero-pets.png" alt="Собака и кошка"
    ...
    <section class="py-10 md:py-12 bg-white">
      <div class="container mx-auto px-4">
        ...
        <div v-else-if="topRegions.length > 0" class="bg-gray-50 p-4 sm:p-6 rounded-lg shadow">
          <ol class="space-y-3">
            <router-link v-for="(region, index) in topRegions" :key="region.id"
              :to="{ name: 'Advertisements', query: { region: region.id } }" custom
              v-slot="{ navigate, isActive, isExactActive }">
              <li @click="navigate" @keypress.enter="navigate" role="link" tabindex="0" :class="[
                ...
                <div class="flex-grow min-w-0">
                  <span class="text-base sm:text-lg font-medium text-gray-800 group-hover:text-green-600 truncate block"
                    :title="region.name">
                    {{ region.name }}
                  </span>
                  <p class="text-xs sm:text-sm text-gray-500">Объявлений: {{ region.ad_count }}</p>
                </div>
                <button @click.stop="toggleFavorite(region.id)"
                  class="flex-shrink-0 p-2 text-gray-400 hover:text-red-500 rounded-full hover:bg-red-50 transition-colors z-10 relative"
                  title="Добавить в избранное (пока не работает)" aria-label="Добавить регион в избранное">
        ...
        <div v-else-if="!loadingRegions && topRegions.length === 0" class="text-center text-gray-500 py-8">
      ...
      <div class="container mx-auto px-4">
        <div class="flex flex-col lg:flex-row items-center gap-6 md:gap-8">
          ...
          <div class="w-full lg:w-1/2">
            <img src="/static/images/map-placeholder.png" alt="Карта приютов"
    ...
    <section class="py-10 md:py-12 bg-gray-100">
      <div class="container mx-auto px-4">
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6 md:gap-8 items-center justify-items-center">
          <img src="/static/images/partners/purina.png" alt="Purina"
            class="h-10 md:h-12 opacity-70 hover:opacity-100 transition-opacity" />
          <img src="/static/images/partners/birulevo.png" alt="Приют Бирюлево"
            class="h-10 md:h-12 opacity-70 hover:opacity-100 transition-opacity" />
          <img src="/static/images/partners/drug-sobaka.png" alt="Drug Sobaka"
            class="h-10 md:h-12 opacity-70 hover:opacity-100 transition-opacity" />
          <img src="/static/images/partners/drug-dlya-druga.png" alt="Друг для друга"
            class="h-10 md:h-12 opacity-70 hover:opacity-100 transition-opacity" />

          <img src="/static/images/partners/iskra.png" alt="Искра"
