  ...
  <div class="bg-gray-50 min-h-screen">
    ...
    <div v-else-if="article" class="article-detail-page">
      <div class="container mx-auto max-w-3xl px-4 pt-8 md:pt-12">
        ...
        <p v-if="deleteError" class="text-red-500 text-sm mb-4">
              ...
              <div class="flex items-start space-x-4">
                ...
                <div class="flex-grow">
                  ...
                  <div v-if="editingArticleCommentId === comment.id">
                    ...
                    <p v-if="editArticleCommentError" class="text-red-500 text-xs mt-1">
                      {{ editArticleCommentError }}
                    </p>
                  </div>
...
const submitArticleComment = async () => {
  ...
  try {
    ...
    articleCommentError.value = axios.isAxiosError(err)
...
const saveEditedArticleComment = async (commentId: number) => {
  ...
  try {
    ...
    editArticleCommentError.value = axios.isAxiosError(err)
      ? `Ошибка: ${err.response?.data?.detail || err.message}`
...
const canManageThisArticle = computed(() => {
  ...
  if (article.value.author?.id === authStore.user.id) {
    if (
      ...
      authStore.user.role_permissions?.can_edit_own_article
    ) {
      return true
    }
  }
...
const handleDeleteArticle = async () => {
  if (!article.value || !canManageThisArticle.value) return

  if (
    confirm(
      `Вы уверены, что хотите удалить статью "${article.value.title}"? Это действие необратимо.`,
    )
  ) {
