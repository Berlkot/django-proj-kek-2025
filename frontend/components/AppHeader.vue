    ...
    <div class="container mx-auto px-4 py-3 flex justify-between items-center">
      ...
      <nav class="hidden md:flex items-center space-x-4 lg:space-x-6 text-gray-700">
        <router-link :to="{ name: 'Advertisements' }" class="hover:text-green-600"
          active-class="text-green-600 font-semibold">Объявления</router-link>
        <router-link :to="{ name: 'Rules' }" class="hover:text-green-600"
          active-class="text-green-600 font-semibold">Правила</router-link>
        <router-link :to="{ name: 'Contacts' }" class="hover:text-green-600"
          active-class="text-green-600 font-semibold">Контакты</router-link>
        <router-link :to="{ name: 'Articles' }" class="hover:text-green-600"
          active-class="text-green-600 font-semibold">Статьи</router-link>
      </nav>
      ...
      <div class="hidden md:flex items-center space-x-3">
        <template v-if="!authStore.isAuthenticated">
          ...
          <button @click="navigateTo({ name: 'Register' })"
        ...
        <template v-else>
          ...
          <Menu as="div" class="relative">
            <MenuButton
              ...
              <img v-if="authStore.user?.avatar_url" :src="authStore.user.avatar_url" alt="Аватар"
            ...
            <transition enter-active-class="transition ease-out duration-100"
              ...
              <MenuItems
                ...
                <div class="px-1 py-1">
                  ...
                  <router-link :to="`/profile/${authStore.user?.id}`"
                    :class="['group flex w-full items-center rounded-md px-2 py-2 text-sm', active ? 'bg-green-500 text-white' : 'text-gray-900']">
                    Мой профиль
                  </router-link>
                  </MenuItem>
                  <MenuItem v-slot="{ active }">
                  <button @click="handleLogout"
                    :class="['group flex w-full items-center rounded-md px-2 py-2 text-sm', active ? 'bg-red-500 text-white' : 'text-gray-900']">
                    Выйти
                  </button>
                  </MenuItem>
                </div>
        ...
        <button @click="handlePostAdClick"
    ...
    <transition enter-active-class="transition ease-out duration-200"
      ...
      <div v-if="isMobileMenuOpen" class="md:hidden absolute top-full left-0 right-0 bg-white shadow-lg z-40">
        <nav class="flex flex-col px-4 pt-2 pb-4 space-y-1 border-t border-gray-200">
          <router-link @click="closeMobileMenu" :to="{ name: 'Advertisements' }"
            class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:bg-gray-100 hover:text-green-600"
            active-class="bg-gray-100 text-green-600">Объявления</router-link>
          <router-link @click="closeMobileMenu" to="/rules"
            class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:bg-gray-100 hover:text-green-600">Правила</router-link>
          <router-link @click="closeMobileMenu" to="/contacts"
            class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:bg-gray-100 hover:text-green-600">Контакты</router-link>
          <router-link @click="closeMobileMenu" :to="{ name: 'Articles' }"
            class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:bg-gray-100 hover:text-green-600"
            active-class="bg-gray-100 text-green-600">Статьи</router-link>
          <hr class="my-2">
          <template v-if="!authStore.isAuthenticated">
            <button @click="handleMobileAuthClick({ name: 'Login' })"
