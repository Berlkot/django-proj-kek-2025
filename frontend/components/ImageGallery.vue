<template>
  <div v-if="photos && photos.length > 0" class="image-gallery">
    <div class="main-image-container mb-3 relative">
      <img
        :src="currentPhotoUrl"
        :alt="`Фото объявления ${currentIndex + 1}`"
      ...
      <!-- Точки для навигации (если много фото) -->
      <div v-if="photos.length > 1 && photos.length <= 7" class="absolute bottom-3 left-1/2 -translate-x-1/2 flex space-x-2">
        <button
    ...
    <div v-if="photos.length > 1" class="thumbnails-container grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2">
      <img
        v-for="(photo, index) in photos"
        :key="photo.id"
        :src="photo.image_url"
...
// Сброс индекса, если фото изменились (например, при переходе на другое объявление)
watch(() => props.photos, () => {
